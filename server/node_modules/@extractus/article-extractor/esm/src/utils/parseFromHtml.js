...
import { getTTR, stripTags, truncateByChar, unique } from "../../deps/jsr.io/@pwshub/bellajs/13.1.1/mod.js";
import { cleanify, countImages } from "./html.js";
import { absolutify as absolutifyUrl, chooseBestUrl, getDomain, isValid as isValidUrl, normalize as normalizeUrls, purify as purifyUrl, } from "./linker.js";
import extractMetaData from "./extractMetaData.js";
import extractWithReadability from "./extractWithReadability.js";
import { execPostParser, execPreParser } from "./transformation.js";
import { defaultAllowedAttributes, defaultAllowedIframeDomains, defaultAllowedTags, } from "../config.js";
const fmtIso = (input) => {
    try {
...
export default async (inputHtml, inputUrl = "", parserOptions = {}) => {
    ...
    const { url, shortlink, amphtml, canonical, description: metaDesc, image: metaImg, author, published, favicon: metaFav, type, } = meta;
    const { wordsPerMinute = 300, descriptionTruncateLen = 210, descriptionLengthThreshold = 180, contentLengthThreshold = 200, allowedTags = defaultAllowedTags, allowedAttributes = defaultAllowedAttributes, allowedIframeDomains = defaultAllowedIframeDomains, } = parserOptions;
    const links = unique([url, shortlink, amphtml, canonical, inputUrl]
        .filter(isValidUrl)
        .map(purifyUrl));
    if (!links.length) {
        return null;
    ...
    const readabilityExtracted = extractWithReadability(preTransformedHtml, bestUrl);
